...
/**
 * now support load a new class into the classloader who load the target class, or transform the target class by
 * bytecode
 * 
...
public class AgentMain {
	...
	public static void agentmain(String filePath, Instrumentation inst) throws Exception, UnmodifiableClassException,
	...
	private static void handle(Instrumentation inst, Class<?> c) throws Exception {
	...
	private static void loadNewClass(Class<?> c) throws Exception {
		URLClassLoader ucl = new URLClassLoader(parameters.getNewClassPath(), c.getClassLoader());
		Logger.info("URLClassLoader " + ucl + " will load " + parameters.getNewClassFullName());
